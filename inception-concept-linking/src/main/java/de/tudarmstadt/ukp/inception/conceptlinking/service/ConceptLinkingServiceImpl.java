    ...
    implements InitializingBean, ConceptLinkingService
{
            ...
            ConceptFeatureValueType aValueType, String aQuery, String aMention)
    {
        ...
        String[] containingTerms = asList(aQuery, aMention).stream()
        ...
        if (containingTerms.length > 0) {
            ...
            containingLabelBuilder.retrieveLabel().retrieveDescription();
            containingDescBuilder.retrieveLabel().retrieveDescription();
            
            List<KBHandle> containingLabelMatches;
            List<KBHandle> containingDescMatches;
            
            if (aKB.isReadOnly()) {
                containingLabelMatches = kbService.listHandlesCaching(aKB, 
                        containingLabelBuilder, true);
                containingDescMatches = kbService.listHandlesCaching(aKB, 
                        containingDescBuilder, true);
            }
            else {
                containingLabelMatches = kbService.read(aKB, 
                        conn -> containingLabelBuilder.asHandles(conn, true));
                containingDescMatches = kbService.read(aKB,  
                        conn -> containingDescBuilder.asHandles(conn, true));
            }
